	...
	<div id="content">
		...
		<div class="container-fluid">
			<div class="row-fluid">
				<div class="span12">
					<div class="alert alert-info">
						您的位置: <strong>站点查询</strong> <a href="#" data-dismiss="alert"
							class="close">×</a>
					</div>
				</div>
			</div>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body" id="selectStations1">
					<p>站点查询(站点名称)</p>
				</div>
				...
				<!-- Table -->
				<table class="table">
					<button type="button" class="btn btn-primary" onclick="showBusByPage(1)">查询</button>
				</table>
			</div>
			...
			<div class="row-fluid">
				<div class="span12">
					<div class="widget-box widget-calendar">
						...
						<div align="center">
							<a href="javascript:showBusByPage(1)" title="首页">首页</a> <a
								href="javascript:showBusByPage(2)" title="上一页">上一页</a> <a
								href="javascript:showBusByPage(3)" title="下一页">下一页</a> <a
								href="javascript:showBusByPage(4)" title="尾页">尾页</a>
						</div>
					</div>
				</div>
			</div>
...
$.post('findAllStations',function(data){
	for(var i=0;i<data.length;i++){
		document.getElementById('selectStations').options.add(new Option(data[i].sname,data[i].sid));
	}
},'json');	
...
function showBusByPage(x) {
 ...
 function showBusLines(pageNum) {
	var bline = $("#selectStations option:selected").val();
	$.post('findAllByPage', {'bline':bline,'page':pageNum,'rows':6,'bname':""}, function(data) {
		$('#showBusLines').html('');
		var str = '';
		if (data.length != 0) {
 ...
 function getBusTotal(){
	var bline = $("#selectStations option:selected").val();
	$.post('getBusTotal',{'bname':"",'bline':bline},function(data){
		$('#total').val(data);
	},'json');
}  
</script>	
